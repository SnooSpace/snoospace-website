...
function App() {
  ...
  const maskFillRef = useRef(null);
  const wipeTextRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // 2. Mouse Tracking for Cursor Glow
    ...
    // 3. SVG Mask Setup (viewBox-based)
    gsap.set(maskGroupRef.current, {
      scale: 120,
      transformOrigin: "443px 107px",
    });

    gsap.set(maskOverlayRef.current, { opacity: 0 });
    gsap.set(maskFillRef.current, { opacity: 0 });

    // 4. Hero Pin Timeline (The GTA Zoom)
    ...
    heroTl
      ...
      .to(comingSoonTextRef.current, { opacity: 0, duration: 0.5 }, 8.5);

    // Stage 3 (9.0s+): The Breakthrough Pop High-End Reveal
    heroTl
      .to(
      ...
      .to(finalLogoRef.current, { opacity: 1, duration: 0.8 }, 9.3)
      .set(whiteLayerRef.current, { opacity: 0 }, 10);

    // Handle Resize Recalculation
    const handleResize = () => {
      gsap.set(maskGroupRef.current, {
        x: window.innerWidth / 2 - 443,
        y: window.innerHeight / 2 - 107,
      });
    };
  ...
  return (
    <div className="app-container" ref={containerRef}>
      ...
      {/* 500vh GTA 6 Zoom HERO Wrapper */}
      <div className="hero-wrapper">
        <div className="hero-pin">
          {/* Layer 1: Bottom (The Stitch Aurora BG) */}
          ...
          {/* New White Transition Layer (Between BG and Mask) */}
          <div ref={maskFillRef} className="hero-layer-white-fill"></div>

          {/* Layer 2: Middle (The SVG Logo Mask) */}
          <div className="hero-layer hero-layer-mask" ref={maskOverlayRef}>
            <svg
          ...
          {/* Layer 3: Top Foreground (Initial Video Content) */}
          <div
            className="hero-layer hero-layer-fg landing-promo-layer"
            ref={videoLayerRef}
          >
            <div className="video-container landing-video-container">
              <video
                ref={landingVideoRef}
                src="/logo-video.mp4"
                autoPlay
            ...
            <div
              ...
              onMouseEnter={() => setIsHoveringLanding(true)}
              onMouseLeave={() => setIsHoveringLanding(false)}
            >
              {/* Play button: visible only when NOT playing */}
              {!isVideoPlaying && (
                <button
                  className="btn-play-reveal font-manrope-semibold"
              ...
              {isVideoPlaying && isHoveringLanding && (
                <button
                  className="btn-play-reveal btn-pause-reveal font-manrope-semibold"
              ...
              <h1
                className="h1-premium font-bc-black coming-soon-text"
          ...
          {/* Crisp Final Logo Layer (Crossfades in at end) */}
          <img
            ref={finalLogoRef}
          ...
          {/* White Glow Transition Layer */}
          <div className="hero-white-glow" ref={whiteLayerRef}></div>

          <div className="hero-wipe-container">
            <h1 className="hero-wipe-text font-bc-bold" ref={wipeTextRef}>
              Re-imagining Connections
            </h1>
          </div>

          {/* Scroll Down Prompt */}
          <div className="initial-scroll" ref={scrollPromptRef}>
            <ChevronDown
              color="#00C6FF"
              size={48}
